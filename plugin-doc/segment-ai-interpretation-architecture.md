...
4. 每次初次解读和追问都调用内置 Live Web Search，并只采用法语、德语或英语网页来源；
5. 将流式回答显示在 Obsidian 真正的右侧功能栏中；
6. 允许围绕同一分段继续追问；
7. 默认不修改译文、不创建笔记、不调用 MCP；只有用户在插件设置中显式开启的 MCP 服务才可进入 Agent thread；
8. 为以后增加模型 API 运行时和更细粒度的 MCP 工具授权留出稳定扩展点。
...
| ADR-007 | MCP 默认拒绝、按服务名显式授权 | 总开关和各服务默认关闭；插件只保存精确服务名，不继承 Codex 全局启用状态 |
...
| ADR-012 | 每个回答必须进行受限语言的外部检索 | `web_search="live"`；来源正文只接受法语、德语或英语，Apps、Plugins 保持禁用，MCP 仅允许插件白名单 |

### 2.1 为什么先选本地 Agent

| 维度 | 本地 Agent（第一版） | 直接模型 API（后续） |
| --- | --- | --- |
| 主要优势 | 可以在只读边界内按需搜索同一研讨班的本地资料、调用内置 Web Search，并保留连续 thread | 发送范围更容易做成完全显式，部署给未安装 Codex 的用户更简单 |
...
6. 回答能够按需检索同一研讨班的其他本地材料，并在每次初次解读和追问中检索法语、德语或英语网页。
...
8. 每次初次解读和追问在作答前至少调用一次内置 Web Search。
9. 外部来源正文只接受法语、德语或英语；排除中文及其他语言网页、机器翻译镜像和非相关语种摘要页。
10. 回答末尾列出“外部检索来源”，包含网页标题、可点击 URL 和语言。
11. Web Search 不可用或没有合格结果时必须明确报告，不得伪造已检索事实。
12. 本地文件检索只限当前研讨班目录；外部网页检索不受此目录限制。
...
- `cwd` 为当前 Vault 根目录；
- 请求中的 `runtimeWorkspaceRoots` 只包含当前 Vault；
...
- App Server 与 thread 配置都显式使用 `web_search="live"`；内置 Web Search 与命令沙箱网络权限彼此独立，因此无需开放 shell 网络；
- 基础指令包含本设计的只读和搜索范围约束；
- 不启用实验性字段，除非有版本检测和回退；
- 不主动持久化敏感诊断信息。
...
`codex-cli 0.145.0` 会在 `readOnly` thread 的响应中把
`runtimeWorkspaceRoots` 回显为空数组，即使请求仍明确传入当前 Vault。响应校验
因此只接受空数组或唯一根目录恰好为当前 Vault；只读模式、当前 Vault `cwd`
和 `approvalPolicy="never"` 仍必须同时成立，任何其他回显路径都拒绝。
...
- 插件侧禁止写入入口；
- 第一版禁用 MCP；
- 内置 Live Web Search 是唯一允许的外部检索能力；
- 基础指令明确只读；
- 回合完成前后检查目标文件未被插件写入。
...
- 默认本地文件搜索范围为当前研讨班目录；
- 外部网页检索不受 Vault 目录限制，但来源正文只接受法语、德语或英语；
...
本节的首要不变量是：**任何没有在插件白名单中同时配置并开启的外部 MCP，在插件启动和正式 Agent thread 中都不得建立连接。** Codex 全局或项目配置中的 `enabled=true` 不能越过这一边界。
...
默认验收必须证明 Agent 没有获得 MCP 工具。实现不能只假定用户没有全局 MCP 配置，而应从有效配置构造受控 thread。
...
当前实现采用“廉价名称发现 + 默认拒绝 + 后台白名单预检”的策略：

1. App Server 子进程显式关闭 Apps、Plugins 等本任务不需要的外部能力，同时显式启用内置 Live Web Search；
2. Obsidian 插件启动且分段 AI 已启用时，异步调用 `config/read`，按当前 Vault 的配置层读取 `config.mcp_servers` 名称；该 RPC 只解析配置，不建立 MCP 连接；
3. 插件设置包含 `segmentAiMcpEnabled=false` 总开关和 `segmentAiMcpEnabledServers=[]` 精确名称白名单；Codex 配置中原本的 `enabled=true` 不构成插件授权；
4. 为 thread 构造临时 `mcp_servers` 覆盖：所有已发现名称先写入 `enabled=false`，仅将“总开关已开且名称在插件白名单中”的服务改为 `enabled=true`，不写回用户的 `config.toml`；
5. 白名单为空时不调用 `mcpServerStatus/list`，也不创建探测 thread，因此不会连接或检查任何 MCP；
6. 白名单非空时，插件在启动后台创建 `ephemeral=true` 的只读探测 thread，并只在这个受限 thread 上调用 `mcpServerStatus/list`。Codex 可能仍返回禁用服务的零能力条目；只有带有 `serverInfo`、工具、资源或资源模板的条目才视为实际暴露。任何非白名单服务出现 `mcpServer/startupStatus/updated` 启动事件或这些运行时能力都会触发失败关闭；
7. 每次创建或恢复正式 Agent thread 前，只重复一次廉价的 `config/read` 以捕获运行期间新增的全局服务，并用同一默认拒绝模板关闭它；不再在用户点击后重复 MCP 状态枚举；
8. 后台检查失败只写入脱敏诊断，不弹启动通知；若发现非白名单能力暴露，则后续 Agent turn 被阻止。

2026-07-26 的同运行时实测中，旧的 threadless `mcpServerStatus/list` 检查 6 个全局服务需要约 `15.2s`；新默认关闭路径完成初始化、账户检查和名称发现约 `0.9s`，检查/连接服务数为 `0`。仅白名单开启 `openaiDeveloperDocs` 时，后台预检约 `6.3s`，实际有连接信息与工具的只有该服务，其余五项均为 `enabled=false` 的零能力条目。

这里的开关是“服务级授权”：开启一个服务会向 Agent 暴露该服务当时提供的全部工具。插件不依据工具名猜测读写风险，也不能替 MCP 服务保证只读；用户只能开启自己信任的服务。工具级细分授权仍属于后续阶段。
...
```ts
type McpServicePolicy = {
  enabled: boolean;            // 总开关，默认 false
  enabledServerNames: string[]; // 精确名称，默认 []
};
```

未来若需要在已授权服务内部继续缩小工具范围，再增加：
...
不能仅依据工具名称包含 `get`、`list` 或 MCP annotation 就判断安全；在工具级策略实现前，开启服务等于授权其全部工具。
...
插件 `data.json` 只保存服务名称、总开关、逐服务选择、最近发现的名称清单和时间；命令、URL、环境变量与凭据仍由 Codex 配置管理，插件不复制这些敏感字段。
...
- 用户切换分段时切换会话；
- 追问默认继承只读、无 MCP、强制外部网页检索和法/德/英来源限制。
...
- 不创建笔记；
- 不修改插件设置以外的文件；
- 默认设置下 Agent 工具表中没有 MCP；显式开启服务后只出现该服务的能力；
- 默认关闭路径只调用 `config/read`，不调用 threadless `mcpServerStatus/list`；
- 白名单预检在 Obsidian 启动后台运行，禁用服务没有 `serverInfo`、工具、资源或资源模板；
- App Server 和 thread 均将 Web Search 模式固定为 `live`；
- 初次解读和追问的提示词都要求只采用法语、德语或英语网页并列出链接；
- 不自动请求权限提升；
- 日志中没有凭据和完整正文；
...
- 强制只读；
- 完成外部能力边界实验：Apps、Plugins 和 MCP 不可用，内置 Live Web Search 可用。
...
- 流式事件和中断工作；
- Agent 不修改工作区；
- 无法证明 Apps、Plugins 和 MCP 已隔离时禁止开始回合；
- Claudian 停用时仍可运行。
...
- 运行时强制只读；
- 当前解读 thread 没有 Apps 或 Plugins；MCP 默认全部关闭，只有插件设置中显式开启的服务可见；内置 Live Web Search 必须可用；
- 每次初次解读和追问只采用法语、德语或英语网页来源，并在回答末尾列出来源；
- 不修改任何译文、原文、术语表或笔记；
- Agent 不可用时明确报错；
...
### 24.2 2026-07-24 CLI 与 Live Web Search 验收

- 全局 Codex CLI 从 `0.144.5` 更新到 npm 官方 `0.145.0`；
- 使用插件实际的 App Server 参数创建 `readOnly` thread 成功；
- 新版响应中的 `runtimeWorkspaceRoots: []` 已纳入失败关闭式兼容校验；
- `web_search="live"` 的真实回合完成，并返回 OpenAI 官方英文 Web Search
  页面标题、可点击 URL 和 `English` 语言标记；
- 命令沙箱仍保持 `networkAccess: false`，Apps、Plugins 和 MCP 仍保持隔离。
