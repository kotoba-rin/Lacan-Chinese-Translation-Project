...
5. MCP 总开关和逐服务开关默认全部关闭。插件启动后会在后台静默读取本机 Codex 的服务名称；只有在插件白名单中显式开启的服务才会被检查并加入 Agent thread。开启服务会授权该服务当前暴露的全部工具，因此只应开启可信服务。
6. 在译文源码模式点击分段旁的“Ф”，或在阅读预览中点击“【分段 ID】 Ф”。
7. 在“会话上限”中选择可同时打开和生成的会话数，范围为 `1–5`，默认 `3`。不同会话可在后台并行生成，单个会话失败不会锁住其他会话。
8. 结果会在右侧的“Lacan AI”视图中流式显示，并默认跟随最新生成内容；向上滚动阅读时会暂停跟随，点击“回到最新”后恢复。顶部标签用于切换会话；“历史”使用独立滚动区域，可重新打开、重命名、删除单条会话或清空全部会话。清空全部前会二次确认，有任务运行时需先停止并等待结束。
9. 追问时按 `Enter` 发送，按 `Shift+Enter` 换行；中文输入法正在组词时不会误发送。每个会话分别保留草稿和滚动位置。
10. “AI 功能”标签只维护一份可直接编辑的“解读提示词”，所有“Ф”会话共用它。需要改变分析方法时，可刷新 Codex 的 `skills/list` 并建立只包含主要 Skill、辅助 Skill 的 Skill 方案；Skill 方案不再保存另一套提示词。“Ф”使用默认 Skill 方案，下拉菜单可为一次新会话改选。
11. 每次初次解读和追问都会使用内置 Live Web Search；外部来源正文只接受法语、德语或英语，并在回答末尾列出标题、链接和语言。

安全边界：

- Agent thread 使用当前 Vault 作为工作目录，并强制使用只读沙箱和 `never` 审批策略。
- Apps、Plugins 始终禁用。MCP 采用默认拒绝：启动时用 `config/read` 发现名称，不连接任何默认关闭服务；只有显式白名单服务会在后台临时只读 thread 中预检。若未授权服务暴露连接信息或能力，插件会停止后续请求。内置 Live Web Search 始终允许且必须使用。
- Web Search 不可用或没有合格的法语、德语、英语来源时，回答必须明确报告，不得以模型记忆冒充检索结果。
