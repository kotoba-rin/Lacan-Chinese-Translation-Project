...
6. 允许围绕同一分段继续追问；
7. 默认不修改译文、不创建笔记、不调用 MCP；
8. 为以后增加模型 API 运行时和只读 MCP 留出稳定扩展点。
...
| ADR-012 | 每个回答必须进行受限语言的外部检索 | `web_search="live"`；来源正文只接受法语、德语或英语，Apps、Plugins 和 MCP 仍保持禁用 |
...
- 运行时强制只读；
- 当前解读 thread 没有 Apps、Plugins 或 MCP；内置 Live Web Search 必须可用；
